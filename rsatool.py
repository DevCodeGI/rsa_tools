...
def save_key(pem_private_key, pem_public_key):
    try:
        with open(key_name + ".key", "w") as f:
            f.write(pem_private_key.decode())

        with open(key_name + ".pub", "w") as f:
            f.write(pem_public_key.decode())

        print(colored(f"Private and public key are generated successfully and saved into '{key_name}.key' and '{key_name}.pub'.", "green"))
    except Exception:
...
def load_private_key(password):
    try:
        with open(key_name + ".key", "rb") as f:
...
def load_public_key():
    try:
        with open(key_name + ".pub", "rb") as f:
...
# Signing the data
def signing_data(message, password):
    try:
...
def verification_data(message, signature):
    try:
        # Load the public key
